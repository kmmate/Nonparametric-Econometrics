...
def npregression_rddlscv(design, searchrange, ydata, runningxdata, cutoff, treatmentabove, poldegree,
    ...
    if design == 'sharp':
        ...
        # restrict the sample used for loss-computing
        # boolean mask
        ...
        # apply mask
        y_res = y_sub[mask1]
        d_res = np.array(d_sub)[mask1]
        runningx_res = runningx_sub[mask1]
        # get size
        n_res = len(y_res)

        # find optimal bandwidth
        # preallocation
        muhat_h = np.empty((n_res, ))
        sse_list = list()
        hround = 0
        ...
        for h in searchrange:
            ...
            for i in range(n_res):
                # if i is control...
                if not d_res[i]:
                    # and the treatment is above cutoff, use only smaller elements than i
                    if treatmentabove:
                        mask2 = runningx_res < runningx_res[i]
                    # and the treatment is below cutoff, use only larger elements than i
                    else:
                        mask2 = runningx_res > runningx_res[i]
                    # to compute muhat_h
                    try:
                        muhat_h[i] = npregression_locpol(x=runningx_res[i], xdata=runningx_res[mask2], ydata=y_res[mask2],
                                                   poldegree=poldegree, kerneltype=kerneltype, kernelorder=kernelorder,
                                                   bandwidth=h)
                ...
                # if i is in treated...
                    ...
                    # and the treatment is above cutoff, use only larger elements than i
                    if treatmentabove:
                        mask2 = runningx_res > runningx_res[i]
                    # and the treatment is blow cutoff, use only elements smaller then i
                    else:
                        mask2 = runningx_res < runningx_res[i]
                    # to compute muhat_h
                    try:
                        muhat_h[i] = npregression_locpol(x=runningx_res[i], xdata=runningx_res[mask2], ydata=y_res[mask2],
                                                     poldegree=poldegree, kerneltype=kerneltype,
            ...
            sse_h = l2loss(yhat=muhat_h, yactual=y_res)
